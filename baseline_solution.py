...
project = 'caffe' # Please specify the system name in order to run the baseline code to obtain the evaluation metric scores for this project
path = f'{project}.csv'
...
out_csv_name = f'content/{project}_NB.csv'
...
for repeated_time in range(REPEAT):
    ...
    grid = GridSearchCV(
        ...
        scoring='f1'  # Using f1 as the metric for model evaluations
    )
...
df=pd.DataFrame([precisions,recalls,f1_scores])
df.index=['Precision','Recalls','F1_Scores']
df.to_csv(f'{project} baseline_metric_scores.csv') # Store the baseline metric values across the 10 runs for the specified project. Can be verified with results shown in report (figures 8,9,10,11,12)
